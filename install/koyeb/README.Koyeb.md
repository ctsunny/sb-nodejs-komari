...
这份文档专门说明如何把当前仓库部署到 Koyeb，并补充说明老版本为什么容易出现下面这类常见报错：
...
老版本里，如果 Koyeb 没有传入 `SERVER_PORT`，并且你也没有手动修改 `LOCAL_SERVER_PORT`，启动时就会直接报：
...
写进了 `start.sh` 顶部，所以直接导入仓库时，通常不额外设置 `SERVER_PORT` 也能启动。
...
或者改成别的值，例如 `3000`、`8080`，但 Koyeb 暴露端口和健康检查端口要与它保持一致。
...
如果你不改端口，这一步可以不填。  
如果你想显式覆盖默认端口，或者为了让配置更直观，也可以添加：

```text
SERVER_PORT=8000
```

> 说明：下面出现的 Komari 地址、token、域名都只是占位示例，请替换成你自己的配置，不要直接照抄示例值。
...
如果你没有自定义端口，就直接保持 `8000`。  
如果你显式设置了 `SERVER_PORT`，这里也要改成同一个值。
...
现在脚本会优先把订阅地址输出为 Argo 域名：

```text
https://ARGO_DOMAIN/sub
```

只有在 Argo 域名暂时没拿到时，才会回退为：
